#########################################################################
# title: HTTPS_Logging.tcl                                              #
# author: Dario Garrido                                                 #
# date: 20200822                                                        #
# description: iRule for logging HTTPS requests                         #
#########################################################################
...
when CLIENTSSL_DATA priority 50 {
	...
	foreach aHeader [split [SSL::payload] "\r\n"] {
		if { $aHeader ne "" } {
			log local0. $aHeader
			set offset 0
		} else {
...
when SERVERSSL_HANDSHAKE priority 50 {
	if {$logging} {
		SSL::collect
	}
}
when SERVERSSL_DATA priority 50 {
    set offset 0
    log local0. "============================================="
